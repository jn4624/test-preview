...
class LibraryServiceSpec extends Specification {
    ...
    def "대출 요청시 도서 상태에 따른 처리 결과를 확인한다."() {
        given:
        def bookRepository = Stub(BookRepository)
    ...
    def "대출에 성공하면 알림이 발송되어야 한다."() {
        given:
        // Stub에는 verify 기능이 없으므로, 알림을 보내는 PushService를 Mock으로 설정
        ...
        1 * pushService.notification(_ as String)
    }

    def "도서 조회 중에 예외가 발생하면 대출 요청시 예외를 던진다."() {
        given:
        def bookRepository = Stub(BookRepository)
        bookRepository.findBookByIsbn(_ as String) >> new RuntimeException("Database error")
